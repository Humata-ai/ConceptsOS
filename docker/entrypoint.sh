...
# Modes ($VPN_MODE):
#
#   control       — headscale only (legacy multi-tenant control plane).
#                   Terminates TLS via Let's Encrypt; :80 (ACME) + :443.
#
#   node          — tailscaled + Next.js. Joins an external headscale via
#                   $HEADSCALE_SERVER_URL + $TS_AUTHKEY. Next.js binds to
#                   the tailnet IP. (Legacy; retired at P3.)
#
#   netbird-node  — netbird agent + Next.js. Joins $NB_MANAGEMENT_URL with
#                   $NB_SETUP_KEY. Next.js binds to the NetBird WgIP on
#                   interface wt0. (NEW — the target end state.)
#
#   all-in-one    — headscale + tailscaled + Next.js, self-registered.
#                   Demo / single-tenant use only.
#
# Admin passthrough for any mode:
#   docker run --rm image headscale users list
#   docker run --rm image netbird status --json
#   docker run --rm image tailscale status
...
if [[ $# -gt 0 ]]; then
  case "$1" in
    headscale|tailscale|tailscaled|netbird|bash|sh|node)
      exec "$@"
      ;;
  esac
fi
...
mkdir -p "$HS_STATE" "$TS_STATE" "$NB_STATE" /var/run/tailscale /var/run/netbird
chmod 0700 "$HS_STATE" "$TS_STATE" "$NB_STATE" || true

log "VPN_MODE=$MODE"

# ---------------------------------------------------------------------------
# Shared helpers (Headscale / Tailscale — legacy stack)
# ---------------------------------------------------------------------------
...
case "$MODE" in
  ...
  *)
    log "unknown VPN_MODE=$MODE (want control|node|netbird-node|all-in-one)"
    exit 2
    ;;
esac
